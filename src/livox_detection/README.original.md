...
stack. One node (`livox_detection_node`), **VoxelNeXt** backend (see
...
| legacy backends | (removed — VoxelNeXt only) | — |
...
will default to a legacy checkpoint path and VoxelNeXt will fail to
load (`load_params_from_file` raises `KeyError('model_state')`), silently
falling back to a generic clustering heuristic. Symptom: a suspiciously
...
| Param | Default | Notes |
|---|---|---|
| `algorithm` | `voxelnext` | `voxelnext` only (legacy values removed) |
