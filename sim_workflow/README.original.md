...
WBC needs only onnxruntime (torch is for the 3D detection node). Verified
in-venv: both policies load, 516→15 inference, finite targets. The
...
**Note:** `detection:=true` launches the 3D detection node (VoxelNeXt),
which needs torch in the *running* interpreter. Under a plain ROS shell it logs
...
`detection_algorithm:=voxelnext` was silently falling back to a generic
clustering heuristic every launch — the launch files were passing the
wrong checkpoint to it regardless of algorithm, which
