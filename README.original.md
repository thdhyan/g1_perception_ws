...
ROS2 Jazzy (laptop) + ROS Foxy (robot) full-stack perception, SLAM, navigation, and control pipeline for Unitree G1 humanoid — Livox Mid-360 LiDAR + RealSense D435I + VoxelNeXt 3D detection + Nav2 + human follower.
...
The original `g1_perception` package (lidar_bridge, ccvnorm, legacy detection nodes) is preserved. See the bottom of this file for its full documentation.
...
### legacy detection nodes

Both subscribed to `/livox/mid360/points` and published `Detection3DArray` + `MarkerArray` using ported `.pt` weights; both were superseded by VoxelNeXt (`pt/voxelnext_nuscenes.pth`).
